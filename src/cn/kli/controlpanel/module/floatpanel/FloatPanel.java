...
public class FloatPanel extends FloatView implements OnClickListener,
	...
	public FloatPanel(Context context) {
		super(context);
		View contentView = LayoutInflater.from(getContext()).inflate(R.layout.control_panel, null);
    ...
    private void loadWidgets(){
    	int[] switchList = {};
    	final SwitchAdapter switchAdapter = new SwitchAdapter(getContext(), switchList);
    	...
    	IWidget widget;
    	for(int i: wigetList){
    		widget = WidgetFactory.createControlbar(getContext(), i);
    		if(widget != null){
    ...
    private void loadTheme(){
		...
		SharedPreferences share = getContext().getSharedPreferences(ThemeSetting.SETTING_PREFERENCES, Context.MODE_PRIVATE);
		if(share == null){
			colorHeader = getContext().getResources().getColor(R.color.translucent_background_dark);
			colorContainer = getContext().getResources().getColor(R.color.translucent_background);
		}else{
			colorHeader = share.getInt(ThemeSetting.THEME_HEADER_COLOR, 
					getContext().getResources().getColor(R.color.translucent_background_dark));
			colorContainer = share.getInt(ThemeSetting.THEME_CONTAINER_COLOR, 
					getContext().getResources().getColor(R.color.translucent_background));
		}
	...
	public void onClick(View view) {
		...
		case R.id.title:
			closePanel();
			Intent titleIntent = new Intent(getContext(), ControlActivity.class);
	...
	public void closePanel() {
		super.closePanel();
//		FloatPanelService.showIndicator(getContext());
	}
